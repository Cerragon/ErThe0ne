...
bool Renderer::Reset(const HRESULT deviceState)
{
	...
	{
		MoveWindow(ErectusMain::appHwnd, ErectusMain::windowPosition[0], ErectusMain::windowPosition[1],
...
{
	d3D9Parameters = {
		.BackBufferWidth = ErectusMain::windowSize[0],
		.BackBufferHeight = ErectusMain::windowSize[1],
		.BackBufferFormat = D3DFMT_A8R8G8B8,
		.BackBufferCount = 1,
		.MultiSampleType = D3DMULTISAMPLE_NONE,
		.MultiSampleQuality = 0,
		.SwapEffect = D3DSWAPEFFECT_DISCARD,
		.hDeviceWindow = ErectusMain::appHwnd,
		.Windowed = TRUE,
